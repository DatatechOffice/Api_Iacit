...
public class ServiceTemperaturaOrvalho {
	...
	public void temperaturaOrvalhoCopy(ArrayList<String> datas, ArrayList<String> estacaoCodigo, ArrayList<String> valorMaximoOrvalho,
			ArrayList<String> valorMinimoOrvalho, ArrayList<String> valorOrvalho) throws IOException {
		...
		Writer writer = Files.newBufferedWriter(Paths.get("C:\\DataFrame\\temperaturaOrvalho.csv"));
			...
			int ii = datas.size();
			for (int i = 0; i < ii; i++) {
				if (valorMaximoOrvalho.get(i).isEmpty()||valorMinimoOrvalho.get(i).isEmpty()||valorOrvalho.get(i).isEmpty()) {
					continue;
				}else {
				String[] jorge = {datas.get(i), valorMaximoOrvalho.get(i), valorMinimoOrvalho.get(i), valorOrvalho.get(i), estacaoCodigo.get(i)};
				jorgin.add(jorge);
				}
			}
