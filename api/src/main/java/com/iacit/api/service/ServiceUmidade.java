...
public class ServiceUmidade {
	...
	public List<Umidade> getByFilter(String estacao, String dataInicial, String dataFinal) throws ParseException {
		...
		List<Umidade> entidades = umidadeRepository.findByest_codigoAndumi_data_hora(idEstacao.getEstCodigo(),
				Timestamp.valueOf(dataInicial + " 00:00:00"), Timestamp.valueOf(dataFinal + " 00:00:00"));
		return entidades;
	}

	public void umidadeCopy(ArrayList<String> datas, ArrayList<String> estacaoCodigo,
			ArrayList<String> valorMaximoUmidade, ArrayList<String> valorMinimoUmidade, ArrayList<String> valorUmidade)
			throws IOException {
		...
		int ii = datas.size();
		for (int i = 0; i < ii; i++) {
			if (valorMaximoUmidade.get(i).isEmpty() || valorMinimoUmidade.get(i).isEmpty()
					|| valorUmidade.get(i).isEmpty()) {
				continue;
			} else {
				String[] jorge = { datas.get(i), valorMaximoUmidade.get(i), valorMinimoUmidade.get(i),
						valorUmidade.get(i), estacaoCodigo.get(i) };
				jorgin.add(jorge);
			}
		}
