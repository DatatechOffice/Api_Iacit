...
@Controller
public class UmidadeRelativaArController {

	@Autowired(required = true)
	private ServiceUmidade umidadeService;

	@PostMapping(value = { "/umidadeRelativaAr" }, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<Umidade>> postFiltroPorData(@RequestBody FilterDataVo data) throws ParseException {
		...
		return listUmidade != null && listUmidade.size() > 0 ? new ResponseEntity<List<Umidade>>(listUmidade, HttpStatus.CREATED)
