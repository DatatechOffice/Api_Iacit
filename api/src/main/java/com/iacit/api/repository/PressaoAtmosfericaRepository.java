...
public interface PressaoAtmosfericaRepository extends JpaRepository<PressaoAtmosferica, Integer> {
	...
	public List<PressaoAtmosferica> findByest_codigoAndpra_data_hora(String estCodigo, Timestamp dataInicial,
			Timestamp dataFinal);

	@Query(value = "copy pressao_atmosferica (pra_data_hora, pra_max, pra_min, pra_nivel_estacao, est_codigo ) from "
			+ "'C:\\dataFrame\\pressao_atmosferica.csv' with delimiter ',' csv header encoding 'iso-8859-1'", nativeQuery = true)
